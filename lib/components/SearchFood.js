...
class SearchFood extends React.Component {
  ...
  render() {
    return (
      <div>
        ...
        {this.state.pagesRemaining && <button onClick={this.loadMore}>Load more...</button>}
      </div>);
  }
  ...
  performSearch = async (searchTerm) => {
    try {
      ...
      this.setState((state) => ({ loading: true, items: newSearch ? [] : state.items, pagesRemaining: false }));
      ...
      this.setState((state) => ({
        ...
        page,
        searchTerm,
        pagesRemaining: pageSize * page < data.count
      }));
    } catch (ex) {
  ...
  searchFood(searchTerm, page) {
    return axios.get(`https://world.openfoodfacts.org/cgi/search.pl?search_terms=${searchTerm}&search_simple=1&action=process&json=1&page_size=${pageSize}&page=${page}`);
  }
