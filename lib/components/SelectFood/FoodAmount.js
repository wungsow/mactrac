...
class FoodAmount extends Component {
  ...
  get total() {
    ...
    this.state.amount && Object.keys(food).forEach(key => total[key] = food[key] * (this.state.amount/100));
    return total;
  }
    ...
    return (
      <form className={style.container} onSubmit={(e) => e.preventDefault()}>
        <div className={style.food}>
          <h2>{food.name}</h2>
          <Macros {...food}></Macros>
          <TextInput value={this.state.amount} number={true} onChange={this.handleChange} name='amount' label='Amount' units={this.state.units} />
